	...
	remove = async() => {
		...
		if (this.state.removeUserIndex !== -1){
			await this.props.deleteUser(
				"users",
				this.props.classes.assignUser[this.state.removeUserIndex].user_id
			);
			if (
				this.props.user &&
										...
										.filter((user: any) => {
											if (!this.state.userFilterText) {
												...
												return (user.user.name || "")
													.toLowerCase()
													.startsWith(this.state.userFilterText.toLowerCase()) || (user.user.role || "")
													.toLowerCase()
													.startsWith(this.state.userFilterText.toLowerCase());
											}
										})
					...
					<div className='mb-16'>
						...
						{this.state.removeUserIndex !== -1 && (
							<span className='f-20 fw-500'>
								...
								Remove user ‘{" "}
								{this.props.classes.assignUser.length > 0 && this.props.classes.assignUser[this.state.removeUserIndex] && this.props.classes.assignUser[this.state.removeUserIndex].user &&
								this.props.classes.assignUser[this.state.removeUserIndex].user.name
									? this.props.classes.assignUser[this.state.removeUserIndex]
											.user.name
									: this.props.classes.assignUser[this.state.removeUserIndex]
											.user.email}{" "}
								’?{" "}
			...
			<div className='class-attendance-body mt-16 '>
				<div className='tableSearch'>
					<div className='textArea'>
						<div className='dash-search-div'>
							<div className='dash-search-icon-div'>
								<SearchIcon sx={{ color: "#808080", fontSize: 16, mr: 0.5 }} />
							</div>
							<input
								className='dash-input-div'
										...
										<tr>
											...
											<td>
												<Dropdown className='more-dropdown'>
													...
													<Dropdown.Menu>
														<Dropdown.Item href={"/admin/school/"+this.id+"/class/" + classe.id}>
															<span>View</span>
														</Dropdown.Item>
