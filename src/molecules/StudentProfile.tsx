...
class StudentProfilePage extends React.Component<IProps, IStates> {
	...
	getDetailAll = async () => {
		...
		await Promise.all([
			this.props.getUserInfo(this.props.user_id, true),
	...
	goGiveBadge = (id: number) => {
		this.props.history.push(
			this.props.defaultPath + "/dashboard/give-badges/" + id
		);
	};
	...
	goToAllComments = (id: any) => {
		...
		let cmdUrl =
			this.props.defaultPath + "/dashboard/all-comments/" + id + "/user";
	...
	goEventDetail = async (event: any) => {
		...
		this.props.history.push(
			this.props.defaultPath + "/event/detail/" + event.id
		);
	};
