...
class CoachStudentAttandPage extends React.Component<IProps, IStates> {
  ...
  createAttend = async () => {
    ...
    await this.props.createAttendance({ users: userList, class_id: parseInt(this.state.classId + "") })
  }
      ...
      let res = tempAttendances.map((attend: any) => {
        return {
          ...
          chooseCallBack: (check: any) => {
            let index = this.state.attendances.findIndex((x: any) => x.id == attend.id)
  ...
  render() {
    ...
    let profile: IProfile = {
      ...
      display_item: [
        ...
        {
          title: 'Time',
          value: viewClass?.start_time
        },
        {
          title: "Coaches",
          value: viewClass?.assign_user.slice(0, 2).map((userInfo: any) => userInfo.user.name || "User").join(", ")
        },
        {
          title: 'No. Student',
    ...
    const { attendances, isfilterDateValid, isfilterDateEmpty, filterDateMsg, filterDate } = this.state
    return (
      <div className="wrapper-mobile bg-w">
        <div className="content-mobile-cus-space col-sm-12">
          <CoachMobileHeader backBtn={true} filterBtn={true} callback={() => {
            this.setState({
              ...this.state,
              modalShow: true
            })
          ...
          <div className="mini-profile pt-24">
            <div className="img-profile">
              <CreateProfile image_url={viewClass?.logo} name={viewClass?.name || "User"} />
            </div>
          ...
          <button type="submit" className="btn btn-primary right w-100" onClick={() => this.createAttend()}>
            Done
          </button>
...
export default connect(mapStateToProps, { getClassAttend, createAttendance })(CoachStudentAttandPage)