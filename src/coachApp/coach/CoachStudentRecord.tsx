        ...
        <div className="content-mobile-cus-space col-sm-12">
          ...
          <div className="mb-8">
            <ListBoxUI title="You" callback={() => { }} callback2={() => { }} more={true} moreText="View Profile">
              <BestScoreBox score={maxObj?.record || "0.00"} title="Personal Best" scoreDate="6 July 2022" />
            </ListBoxUI>
          </div>
          <div className="mb-8">
            <ListBoxUI title={'history'} callback={() => { }} more={false} noBtn={true}>
              {eventRecords.length > 0 ?
                <>
                  {eventRecords.map((record: any, index: any) => (
                    <ListItem text={record.record} secondryText={true} callback={() => { }} key={`classOb${index}`}>
                      <>
                        <CalendarTodayOutlinedIcon fontSize='small' />
                        <label>{moment(record.created_date).format("DD MMM YYYY")}</label>
                      </>
                    </ListItem>
                  ))}
                </>
